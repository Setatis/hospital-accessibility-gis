"""离线合成上海市行政区、医院、路网的演示数据。

当网络不可用或 OSM 下载失败时，main.py 会回退到本模块生成合成数据，
保证整个分析流程可离线跑通、结果可复现。

数据策略：
- 行政区：将上海研究区多边形网格化，按“到各区种子点最近距离”归属聚合为 16 个区，
  保证无缝、不重叠、铺满整个研究区。
- 医院：按各区种子点附近随机散布若干医院点，城区（中心区）密度更高。
- 路网：生成经纬网格状道路线，供演示用的简化路网分析使用。
"""
